#!/usr/bin/env python3
# monitor.py - SIMPLE 殘差即時監控（Phase 1 Mixing Length 版本）
# 修改說明：
#   原版觸發條件為 TKED_system（k-ε 專用），Phase 1 無 TKE/TKED 導致圖表空白
#   修改為：找到 Pressure equation 就 append，適用 Phase 1 Mixing Length
    ...
    try:
        with open(LOG_FILE, 'r') as f:
            for raw_line in f:
                ...
                if m and 'x' in current and 'y' in current and 'z' in current:
...
def update(frame):
    ...
    ax1.axhline(1e-4, color='gray', linestyle='--', label='target 1e-4')
    ...
    ax2.axhline(1e-4, color='gray', linestyle='--', label='target 1e-4')
