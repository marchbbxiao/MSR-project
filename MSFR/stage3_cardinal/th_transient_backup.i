...
# pressure_tag = "pressure_grad"  # SIMPLE only
...
[AuxVariables]
  ...
  [mu_t]
    ...
    type = MooseVariableFVReal
    # initial_condition = 0.162  # overridden by initial_from_file_var
    two_term_boundary_expansion = false
  []
  [yplus]
    # 無因次壁面距離 y⁺ [-]
...
[FVKernels]
  ...
  # p_diffusion and p_source are SIMPLE-only, replaced by INSFVMassAdvection
  [mass]
  ...
  [vel_x_pressure]
    ...
    momentum_component = x
    pressure = pressure
    # extra_vector_tags = ${pressure_tag}  # SIMPLE only
  []
  ...
  # type = INSFVEnergyTimeDerivative  # commented: fix pending
  #   variable = T_fluid
  #   rho = ${rho}
  #   cp = ${cp}
  # []
