...
[AuxKernels]
# ---- 純層流診斷版：AuxKernels 依賴 TKE/TKED，一併停用 ----
  #[compute_mu_t]
    # 計算湍流動黏度：μt = Cμ × k²/ε
...
[Executioner]
  ...
  print_fields = false
  # 即使未收斂也繼續（不強制中止），讓 checkpoint 能正常寫出
...
[FVKernels]
  ...
  # []
# ---- Stokes 診斷版：移除對流項，線性化方程式確認擴散流場收斂 ----
  # [vel_x_advection]
...
# ============================================================
[Outputs]
  # Exodus 格式輸出（可用 Paraview 視覺化）
  exodus = true
  # Checkpoint：儲存完整求解器狀態，供後續作為初始條件
  [checkpoint]
    type = Checkpoint
    num_files = 2
  []
